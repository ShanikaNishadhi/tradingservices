...
class SimpleTrendsDatabase:
    ...
    def _init_database(self):
        ...
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS simple_trends_memecoins_orders (
        ...
        # Create indexes
        cursor.execute("""
            CREATE INDEX IF NOT EXISTS idx_stmc_orders_symbol_status
            ON simple_trends_memecoins_orders(symbol, status)
        """)

        # Strategy state table - stores min/max prices per symbol
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS simple_trends_memecoins_state (
    ...
    def add_order(self, symbol: str, side: str, quantity: Decimal, entry_price: Decimal,
        ...
        cursor.execute("""
            INSERT INTO simple_trends_memecoins_orders
    ...
    def update_order_stop_orders(self, order_db_id: int, stop_loss_order_id: Optional[str] = None,
        ...
        cursor.execute("""
            UPDATE simple_trends_memecoins_orders
    ...
    def get_open_orders(self, symbol: str) -> List[Dict]:
        ...
        cursor.execute("""
            SELECT * FROM simple_trends_memecoins_orders
    ...
    def save_state(self, symbol: str, min_price: Decimal, max_price: Decimal):
        ...
        cursor.execute("""
            INSERT INTO simple_trends_memecoins_state (symbol, min_price, max_price, updated_at)
            VALUES (%s, %s, %s, NOW())
            ON CONFLICT (symbol)
            DO UPDATE SET
    ...
    def get_state(self, symbol: str) -> Optional[Dict]:
        ...
        cursor.execute("""
            SELECT * FROM simple_trends_memecoins_state
            WHERE symbol = %s
        """, (symbol,))
